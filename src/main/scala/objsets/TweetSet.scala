...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  def ascendingByRetweet0(accu: Trending): Trending = {
    val m = this.findMin
    this.remove(m).ascendingByRetweet0(accu + m)
  }
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    val h = this.head
    if (p(h)) this.tail.filter0(p, accu.incl(h)) else this.tail.filter0(p, accu)
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(s => t.text.contains(s)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(s => t.text.contains(s)))
